...
function loadLanguage(a) {
  ...
  if (!(0 >= a) && lastCurrentCardId !== a) {
    ...
    if (cardsTemp[a]) {
      ...
      var pendulum = cardsTemp[a][language].pendulum
        ? `←${card.lscale}【Pendulum Effect】${card.lscale}→<br> ${cardsTemp[a][language].pendulum} <br><br> 【Monster Effect】<br>`
        : "";

      document.getElementById("card-description").innerHTML =
        pendulum + cardsTemp[a][language].lore;
    } else {
...
function createSelect() {
  // Cargar lista de lenguajes predeterminados
